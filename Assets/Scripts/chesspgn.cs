...
public class ChessGameManager : MonoBehaviour
{
    ...
    public void RecordMove(string move, ChessPiece[,] board)
    {
        moves.Add(move);

        // Generate PGN after each move
        PGN = GeneratePGN(board);
        Debug.Log("PGN:\n" + PGN);
    }

    // Method to generate PGN from game info and moves
    private string GeneratePGN(ChessPiece[,] board)
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            string move = moves[i];
            if (DisambiguationNeeded(move, board))
            {
                move = AddDisambiguation(move, board);
            }
    ...
    private bool DisambiguationNeeded(string move, ChessPiece[,] board)
    {
        ...
        {
            case 'R': // Rook
                pieceCount = CountPiecesCanReachDestination<Rook>(destination, board);
                break;
            case 'B': // Bishop
                pieceCount = CountPiecesCanReachDestination<Bishop>(destination, board);
                break;
            case 'N': // Knight
                pieceCount = CountPiecesCanReachDestination<Knight>(destination, board);
                break;
            case 'P': // Pawn
                pieceCount = CountPiecesCanReachDestination<Pawn>(destination, board);
                break;
            default:
    ...
    private int CountPiecesCanReachDestination<T>(string destination, ChessPiece[,] board) where T : ChessPiece
    {
        int count = 0;
        ...
        for (int x = 0; x < tileCountX; x++)
        {
            for (int y = 0; y < tileCountY; y++)
            {
                ...
                if (piece != null && piece.GetType() == typeof(T)) // Check if the piece is of the specified type
                {
                    ...
                    List<Vector2Int> validMoves = piece.GetPossibleMoves(ref board, tileCountX, tileCountY);

                    // Check if any of the valid moves of the piece reach the destination square
                    foreach (Vector2Int move in validMoves)
                    {
                        if (move.ToString() == destination)
                        {
                            count++;
    ...
    private string AddDisambiguation(string move, ChessPiece[,] board)
    {
        // Extract destination square from the move
        string destination = move.Substring(move.Length - 2);
        ...
        switch (pieceType)
        {
            ...
            case 'N': // Knight
                ...
                FindOriginSquare(pieceType, destination, out originRank, out originFile, board);
                disambiguatedMove = originFile + destination;
                break;
            case 'P': // Pawn
                string originRankPawn, originFilePawn;
                FindOriginSquare(pieceType, destination, out originRankPawn, out originFilePawn, board);
    ...
    private void FindOriginSquare(char pieceType, string destination, out string rank, out string file, ChessPiece[,] board)
    {
        // Iterate through the board to find pieces of the specified type
        for (int x = 0; x < tileCountX; x++)
        {
            ...
            {
                ChessPiece piece = board[x, y];
                if (piece != null && (char)piece.type == pieceType) // Check if the piece is of the specified type
                {
                    // Get possible moves for the piece
                    List<Vector2Int> validMoves = piece.GetPossibleMoves(ref board, tileCountX, tileCountY);
